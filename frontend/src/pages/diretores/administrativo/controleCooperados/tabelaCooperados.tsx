...
// TODO: Substituir por chamada à API no futuro
...
/**
 * Página principal de listagem de Cooperados.
 * Gerencia a tabela de exibição e os modais de entrada (Novo Cadastro ou Importação via PDF).
 */
export function CooperadoIndexPage() {
  ...
  /**
   * Transita do modal de escolha inicial para o modal específico de upload de PDF.
   */
  ...
  /**
   * Simula o processamento do arquivo PDF importado.
   */
            ...
            {cooperadosMocados.map((cooperado) => (
              <tr key={cooperado.id} className="hover:bg-gray-50 transition-colors group">
                ...
                <td className="p-4 text-center">
                  <button className="text-gray-400 hover:text-[#00A859] hover:bg-emerald-50 p-2 rounded-full transition-all">
      ...
      {isModalInitialOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm transition-opacity">
          <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-sm relative flex flex-col items-center animate-in fade-in zoom-in-95 duration-200">
            <button onClick={() => setIsModalInitialOpen(false)} className="absolute -top-3 -right-3 bg-red-500 hover:bg-red-600 text-white rounded-full w-8 h-8 flex items-center justify-center shadow-lg transition-transform hover:scale-105">
              <X size={18} strokeWidth={3} />
            </button>
            <h2 className="text-xl font-bold text-gray-800 mb-6 text-center">Tipo de Cadastro</h2>
            <div className="flex flex-col gap-4 w-full">
              <button onClick={() => navigate('/cooperados/novo')} className="bg-[#00A859] hover:bg-emerald-700 text-white font-semibold py-3 px-4 rounded-xl shadow-md transition-all active:scale-95">
                Cadastrar novo cooperado
              </button>
              <button onClick={handleOpenPdfUpload} className="bg-white border-2 border-[#00A859] text-[#00A859] hover:bg-emerald-50 font-semibold py-3 px-4 rounded-xl shadow-sm transition-all active:scale-95">
      ...
      {isUploadPdfModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm transition-opacity">
          <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md relative flex flex-col items-center animate-in fade-in zoom-in-95 duration-200">
            <button onClick={() => setIsUploadPdfModalOpen(false)} className="absolute -top-3 -right-3 bg-red-500 hover:bg-red-600 text-white rounded-full w-8 h-8 flex items-center justify-center shadow-lg transition-transform hover:scale-105">
              <X size={18} strokeWidth={3} />
            </button>
            ...
            <div 
              onClick={() => setPdfFileName(pdfFileName ? null : 'ficha_cadastro_antigo.pdf')}
              className={`w-full border-2 border-dashed rounded-xl p-8 flex flex-col items-center justify-center cursor-pointer transition-all mb-8 group ${pdfFileName ? 'border-[#00A859] bg-emerald-50' : 'border-[#00A859]/40 bg-emerald-50/50 hover:bg-emerald-50'}`}
            >
              {pdfFileName ? (
                <>
                  <FileText className="text-[#00A859] mb-3" size={36} />
                  <span className="text-sm font-bold text-emerald-800 text-center">{pdfFileName}</span>
            ...
            <div className="flex w-full gap-3">
              <button onClick={() => setIsUploadPdfModalOpen(false)} className="w-1/2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-3 px-4 rounded-xl shadow-sm transition-all active:scale-95">
                Cancelar
              </button>
              <button onClick={handleProcessarPdf} disabled={!pdfFileName} className={`w-1/2 font-bold py-3 px-4 rounded-xl shadow-md transition-all active:scale-95 ${pdfFileName ? 'bg-[#00A859] hover:bg-emerald-700 text-white' : 'bg-gray-300 text-gray-500 cursor-not-allowed shadow-none'}`}>
                Processar
              </button>
            </div>
          </div>
